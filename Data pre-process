...
def run_totalseg_total(dicom_dir: Path, out_dir: Path, fast: bool):
    ...
    print(f"[Done] TotalSeg output -> {out_dir}")
...
def build_bone_mask(
    ...
    if save_preview_png:
        mid = bone_mask.shape[-1] // 2
        plt.figure(figsize=(5, 5))
        plt.imshow(bone_mask[..., mid], cmap="gray")
        plt.title("Bone mask (mid slice)")
        plt.axis("off")
        png_path = out_path.parent / "bone_preview_mid_slice.png"
        plt.tight_layout()
...
def proj_img_main(ct_body_zyx, ct_bone_zyx):
    ...
    plt.subplot(1, 3, 1)
    plt.imshow(P_low_y_stretched, cmap="gray")
    plt.title("Low energy: P_y")
    plt.axis("off")

    plt.subplot(1, 3, 2)
    plt.imshow(P_high_y_stretched, cmap="gray")
    plt.title("High energy: P_y")
    plt.axis("off")

    plt.subplot(1, 3, 3)
    plt.imshow(Pathlength_y_stretched, cmap="gray")
    plt.title("Pathlength projection")
    plt.axis("off")
